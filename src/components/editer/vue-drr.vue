...
export default {
  ...
  methods: {
    ...mapActions('editer', ['toggleEditState']),
    initial () {
      this.elmX = this.x
      this.elmY = this.y
      this.elmW = this.w
    ...
    lockHandler(){ // 双击文本可编辑
      this.enabled = !1
      this.toggleEditState({ index: this.zIndex, status: !0 })
    ...
    handleMove (e) {
      ...
      if (this.resizing) {
        ...
        this.$emit('resizing', this.zIndex, this.left, this.top, this.width, this.height)
    ...
    handleUp (e) {
      ...
      if (this.resizing) {
        this.resizing = false
        this.$emit('resizestop', this.zIndex, this.left, this.top, this.width, this.height)
      }
      if (this.dragging) {
        this.dragging = false
